...
def setup_config():
    ...
    sindhi_agent=Agent(name="sindhi_agent",instructions="You translate the user message to sindhi", handoff_description="An english to sindhi translator",
        model=model)
...
async def main(message: cl.Message):
    ...
    result = await Runner.run(triage_agent, str(history), run_config=config)
    ...
    history.append({"role": result.last_agent.name, "content": response_content})
...
def start():
    subprocess.run(["uv","run","chainlit","run","src/agent_as_tool/main.py","-w"])